...
자주 발생하는 오류 3가지 요약입니다. 전체 목록은 **[docs/TROUBLESHOOTING.md](./docs/TROUBLESHOOTING.md)** 를 참고하세요.

| 증상 | 해결 |
|------|------|
| 학습 중 `OverflowError: Python int too large to convert to C int` | tokenizer_processor.py에 `clamp(-6.0, 6.0)` 추가 (이미 적용됨) |
| Pi0-FAST가 garbage 토큰을 뱉는 경우 | Pi0(flow-matching)으로 전환: `./train.sh ... "" "" pi0` |
| `RuntimeError: Offset increment outside graph capture` | `torch.compile(mode="max-autotune")` 사용 (pi0_server_v2에 적용됨) |
